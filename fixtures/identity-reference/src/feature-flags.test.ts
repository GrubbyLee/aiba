...
const definition: FeatureFlagDefinition = {
  key: "vehicle.beta", tenantId: "tenant-a", enabled: true, revision: 3, salt: "trusted-rollout-salt",
...
describe("feature flags reference boundary", () => {
  it("evaluates trusted target attributes before rollout", async () => {
    const result = await service().evaluate({ tenantId: "tenant-a", subjectId: "user-1", attributes: { plan: "enterprise" } }, { flagKey: "vehicle.beta", expectedRevision: 3 });
  ...
  it("is deterministic for one subject and revision", async () => {
    ...
    expect(await service().evaluate(context, { flagKey: "vehicle.beta" })).toEqual(await service().evaluate(context, { flagKey: "vehicle.beta" }));
  });

  it("rejects unknown tenant scope and stale policy revisions", async () => {
    await expect(service().evaluate({ tenantId: "tenant-b", subjectId: "user-1", attributes: {} }, { flagKey: "vehicle.beta" })).rejects.toThrow("flag-unavailable");
    await expect(service().evaluate({ tenantId: "tenant-a", subjectId: "user-1", attributes: {} }, { flagKey: "vehicle.beta", expectedRevision: 2 })).rejects.toThrow("policy-revision-conflict");
  });

  it("uses an explicit disabled variant", async () => {
    await expect(service({ ...definition, enabled: false }).evaluate({ tenantId: "tenant-a", subjectId: "user-1", attributes: {} }, { flagKey: "vehicle.beta" })).resolves.toMatchObject({ enabled: false, variant: "off", reason: "disabled" });
  });
  ...
  it("rejects rollout allocations beyond the fixed bucket", async () => {
    ...
    await expect(service(invalid).evaluate({ tenantId: "tenant-a", subjectId: "user-1", attributes: {} }, { flagKey: "vehicle.beta" })).rejects.toThrow("flag-policy-invalid");
  });
});
