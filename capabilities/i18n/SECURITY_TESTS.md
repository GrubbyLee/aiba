...
| keys-are-namespaced | Translation keys are namespaced and bounded | error |
| interpolation-is-sanitized | Parameter interpolation is sanitized | critical |
| catalog-is-revisioned | Catalogs are revisioned and immutable per release | error |
| fallback-is-deterministic | Fallback chain is deterministic and bounded | warning |
| plural-rules-are-validated | Plural rules are validated against locale | warning |
