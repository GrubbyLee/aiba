...
AIBA 是面向 AI Agent 的能力基础设施。它不是管理后台模板、可视化页面搭建器、
固定的全栈框架，也不是提示词集合。AIBA 向 Agent 提供稳定、有版本的软件行为定义；
Agent 按项目已有的技术栈和设计语言完成适配；确定性的 Core 命令负责检查、验证、
追踪和升级结果。

**一句话划分责任：** Agent 写代码，AIBA 定契约、算哈希、记回执、保升级。

它把"应用必须保证什么"与"代码和界面如何实现"分开。生成代码归项目所有。

### "证据与来源验证"到底验证了什么

当 AIBA 说一个能力"验证通过"时，它确认了这些事：

- 能力声明触及的每个文件都在预期位置存在；
- 每个声明文件的内容哈希与 finalize 时记录的一致；
- 安装回执、血缘链、依赖图完整未被篡改；
- 没有任何必需不变量的证据文件被删除或改动。

AIBA **本身不**执行应用测试，也不直接证明运行时行为。
...
3. **Application Blueprint**描述项目自己的资源、工作流、授权意图、事件、界面意图、
   验收证据和 Agent 写入范围。Core 将其编译为确定性的能力与任务计划，但不执行任务，
   也不生成绑定框架的代码。
4. **应用 Solution**精确绑定一组能力的版本、内容哈希和依赖顺序。每项能力仍须独立通过
...
例如，Agent 可以在具体项目中组合 `form-engine`、`data-dict`、`file-assets`、`tags`、
`inbox` 和 `i18n`，但无需引入统一页面布局。每项能力仍独立保留契约和验证，实际 API、
存储与界面由 Agent 按宿主项目技术栈实现。

只有当某个精确组合本身可以被广泛复用时，才应制作 Solution。把一批无关功能或某个项目
专有的数据模型打包在一起，不是 Solution。
